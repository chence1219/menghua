 ...
 */
 ...
 void McpServer::AddCommonTools() {
     ...
     AddTool("self.get_device_status",
         "Provides the real-time information of the device, including the current status of the audio speaker, screen, network, etc.\n"
         "Use this tool for: \n"
     ...
     AddTool("self.audio_speaker.set_volume",
         ...
         [&board](const PropertyList& properties) -> ReturnValue {
             ...
             if (app.GetAecMode() != kAecOff && volume > 80) {
                 volume = 80;
             }
#endif
     ...
     if (display && !display->GetMode().empty()) {
         AddTool("self.screen.set_mode",
             "Set the display mode of the screen. The mode can be 'normal' (文字模式/text mode) or 'pet' (宠物模式/pet mode with GIF animations).",
             PropertyList({
                 Property("mode", kPropertyTypeString)
             }),
             [display](const PropertyList& properties) -> ReturnValue {
                 display->SetMode(properties["mode"].value<std::string>().c_str());
                 return true;
             });
    }
