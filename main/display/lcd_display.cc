...
LV_IMG_DECLARE(gif_listen);
LV_IMG_DECLARE(sleep1);
LV_IMG_DECLARE(sleep2);
LV_IMG_DECLARE(LOGO);
LV_IMG_DECLARE(buxie);
LV_IMG_DECLARE(love);
...
LcdDisplay::LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, DisplayFonts fonts, int width, int height)
    ...
    current_mode_name_ = settings.GetString("mode", "normal");
    ...
    // Update the mode
    if (current_mode_name_ == "normal") {
        current_mode_ = false;
    }
    else if (current_mode_name_ == "pet") {
        current_mode_ = true;
    }
}
...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
                             ...
                             DisplayFonts fonts)
    : LcdDisplay(panel_io, panel, fonts, width, height) {  // 调用基类 LcdDisplay 构造函数初始化基本属性（宽、高、字体、面板句柄）

    // -------------------- Step 1: 清屏为白色 --------------------
    std::vector<uint16_t> buffer(width_, 0xFFFF); // 创建一行的缓冲区，填充白色 (0xFFFF)
    for (int y = 0; y < height_; y++) {
        esp_lcd_panel_draw_bitmap(panel_, 0, y, width_, y + 1, buffer.data()); // 每行绘制白色
    }

    // -------------------- Step 2: 打开显示屏 --------------------
    ESP_LOGI(TAG, "Turning display on");
    ESP_ERROR_CHECK(esp_lcd_panel_disp_on_off(panel_, true)); // 打开 LCD 显示

    // -------------------- Step 3: 初始化 LVGL 库 --------------------
    ESP_LOGI(TAG, "Initialize LVGL library");
    lv_init(); // 初始化 LVGL 核心库

    // -------------------- Step 4: 初始化 LVGL 驱动端口 --------------------
    ESP_LOGI(TAG, "Initialize LVGL port");
    lvgl_port_cfg_t port_cfg = ESP_LVGL_PORT_INIT_CONFIG(); // 获取默认配置
    port_cfg.task_priority = 1;         // 设置 LVGL 任务优先级
    port_cfg.timer_period_ms = 50;      // 设置 LVGL 刷新周期（ms）
    lvgl_port_init(&port_cfg);          // 初始化 LVGL 硬件抽象层

    // -------------------- Step 5: 添加 LCD 显示器到 LVGL --------------------
    ESP_LOGI(TAG, "Adding LCD display");
    const lvgl_port_display_cfg_t display_cfg = {
        .io_handle = panel_io_,          // 面板 IO 句柄
        .panel_handle = panel_,          // 面板对象句柄
        .control_handle = nullptr,       // 可选控制句柄，未使用
        .buffer_size = static_cast<uint32_t>(width_ * 20), // LVGL 缓冲区大小（可根据需求调整）
        .double_buffer = false,          // 是否启用双缓冲
        .trans_size = 0,                 // 传输块大小，0 表示自动
        .hres = static_cast<uint32_t>(width_),   // 水平分辨率
        .vres = static_cast<uint32_t>(height_),  // 垂直分辨率
        .monochrome = false,             // 是否单色显示
        .rotation = {                     // 屏幕旋转配置
            .swap_xy = swap_xy,           // 交换 X/Y
            .mirror_x = mirror_x,         // X 镜像翻转
            .mirror_y = mirror_y,         // Y 镜像翻转
        },
        .color_format = LV_COLOR_FORMAT_RGB565,  // 颜色格式
        .flags = {                    // 额外标志
            .buff_dma = 1,           // 使用 DMA 加速缓冲区传输
            .buff_spiram = 0,        // 不使用 PSRAM 缓冲
            .sw_rotate = 0,          // 不使用软件旋转
            .swap_bytes = 1,         // 字节交换，确保颜色正确
            .full_refresh = 0,       // 非全屏刷新
            .direct_mode = 0,        // 非直接模式
        },
    };

    display_ = lvgl_port_add_disp(&display_cfg); // 将显示器注册到 LVGL
    if (display_ == nullptr) {
        ESP_LOGE(TAG, "Failed to add display"); // 注册失败则输出错误日志
        return;
    }

    // -------------------- Step 6: 设置显示偏移（可选） --------------------
    if (offset_x != 0 || offset_y != 0) {
        lv_display_set_offset(display_, offset_x, offset_y); // 设置显示器坐标偏移
    }

    // -------------------- Step 7: 创建 UI 界面 --------------------
    SetupUI(); // 调用基类或本类方法创建 LVGL 界面元素
}
...
#else
void LcdDisplay::SetupUI() {
    DisplayLockGuard lock(this); // 锁屏幕，防止并发修改 LVGL 对象

    /* -------------------- 屏幕基础设置 -------------------- */
    auto screen = lv_screen_active(); // 获取当前活动屏幕
    lv_obj_set_style_text_font(screen, fonts_.text_font, 0);         // 设置默认字体
    lv_obj_set_style_text_color(screen, current_theme_.text, 0);     // 设置默认文本颜色
    lv_obj_set_style_bg_color(screen, current_theme_.background, 0); // 设置屏幕背景颜色

    /* -------------------- 主容器 Container -------------------- */
    container_ = lv_obj_create(screen);                   // 创建容器
    lv_obj_set_size(container_, LV_HOR_RES, LV_VER_RES);  // 容器大小覆盖全屏
    lv_obj_set_flex_flow(container_, LV_FLEX_FLOW_COLUMN); // 垂直布局
    lv_obj_set_style_pad_all(container_, 0, 0);          // 内边距 0
    lv_obj_set_style_pad_top(container_, 10, 0);         // 顶部内边距 10
    lv_obj_set_style_border_width(container_, 0, 0);     // 无边框
    lv_obj_set_style_pad_row(container_, 0, 0);          // 行间距 0
    lv_obj_set_style_bg_color(container_, lv_color_black(), 0); 
    lv_obj_set_style_bg_opa(container_, LV_OPA_COVER, 0); // 背景不透明
    lv_obj_set_style_border_color(container_, current_theme_.border, 0); // 边框颜色
    lv_obj_set_scrollbar_mode(container_, LV_SCROLLBAR_MODE_OFF); // 禁用滚动条

    /* -------------------- 状态栏 Status Bar -------------------- */
    status_bar_ = lv_obj_create(container_);  
    lv_obj_set_size(status_bar_, LV_HOR_RES, fonts_.text_font->line_height); // 高度 = 字体高度
    lv_obj_set_style_radius(status_bar_, 0, 0);  // 无圆角
    lv_obj_set_style_bg_color(status_bar_,  lv_color_black(), 0); // 背景黑色
    lv_obj_set_style_text_color(status_bar_, lv_color_white(), 0); // 白色文字

    /* -------------------- Overlay 层 (GIF层) -------------------- */
    overlay_container = lv_obj_create(container_);  
    lv_obj_remove_style_all(overlay_container);   // 清除所有默认样式
    lv_obj_set_size(overlay_container, LV_HOR_RES, LV_VER_RES);  // 全屏
    lv_obj_set_style_bg_color(overlay_container, lv_color_black(), 0);  
    lv_obj_set_style_bg_opa(overlay_container, LV_OPA_COVER, 0); // 不透明
    lv_obj_align(overlay_container, LV_ALIGN_CENTER, 0, 0);       // 居中
    ...
    lv_obj_align(gif_label_, LV_ALIGN_CENTER, 0, -35); // GIF向上偏移 35 像素
    lv_obj_add_flag(gif_label_, LV_OBJ_FLAG_HIDDEN);    // 默认隐藏
    ...
    lv_obj_set_style_pad_all(content_, 5, 0);          // 内边距 5
    lv_obj_set_style_bg_color(content_, lv_color_black(), 0);  
    lv_obj_set_style_border_width(content_, 0, 0);          
    lv_obj_set_flex_flow(content_, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_style_margin_top(content_, 50, 0);  // 整体下移 30 px
    lv_obj_set_flex_align(content_, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_SPACE_EVENLY); 

    /* -------------------- 情绪标签 Emotion Label -------------------- */
    emotion_label_ = lv_label_create(content_);
    lv_label_set_text(emotion_label_, "😶");            // 初始表情
    lv_obj_set_style_text_font(emotion_label_, &font_awesome_30_4, 0); // 大字体
    lv_obj_set_style_text_color(emotion_label_, current_theme_.text, 0); 
    lv_obj_set_width(emotion_label_, LV_HOR_RES);  
    lv_obj_set_height(emotion_label_, 120); 
    lv_obj_set_style_text_align(emotion_label_, LV_TEXT_ALIGN_CENTER, 0); 
    lv_obj_align(emotion_label_, LV_ALIGN_CENTER, 0, 0); // 居中
    lv_obj_set_style_bg_color(emotion_label_, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(emotion_label_, LV_OPA_COVER, 0); 
    lv_obj_set_style_radius(emotion_label_,0, 0); 
    lv_obj_set_style_pad_all(emotion_label_, 25, 0); 
    lv_obj_set_style_border_width(emotion_label_, 0, 0);
    lv_obj_add_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN); // 默认隐藏

    /* -------------------- 预览图片 Preview Image -------------------- */
    preview_image_ = lv_image_create(content_);
    lv_image_set_src(preview_image_, &LOGO); // 设置图片资源
    lv_obj_set_size(preview_image_, width_ * 0.5, height_ * 0.5); // 缩小 50%
    lv_obj_set_parent(preview_image_, lv_scr_act());
    lv_obj_align(preview_image_, LV_ALIGN_CENTER, 0, -20);   // 上移 20 px

    /* -------------------- 聊天消息 Chat Message -------------------- */
    chat_message_label_ = lv_label_create(content_);
    lv_label_set_text(chat_message_label_, "");  
    lv_obj_set_width(chat_message_label_, LV_HOR_RES - 10); // ✅留点内边距
    lv_obj_set_height(chat_message_label_, LV_SIZE_CONTENT); // ✅自适应高度
    lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_WRAP); // ✅自动换行
    lv_obj_set_style_text_align(chat_message_label_, LV_TEXT_ALIGN_CENTER, 0); 
    lv_obj_set_style_text_color(chat_message_label_, lv_color_white(), 0); 
    lv_obj_set_style_text_font(chat_message_label_, &font_puhui_20_4, 0);
    lv_obj_set_style_bg_color(chat_message_label_, lv_color_black(), 0); 
    lv_obj_set_style_bg_opa(chat_message_label_, LV_OPA_COVER, 0); 
    lv_obj_set_style_pad_all(chat_message_label_, 18, 0);
    lv_obj_set_style_margin_top(chat_message_label_, 20, 0);

    /* -------------------- 状态栏内部元素 -------------------- */
    lv_obj_set_flex_flow(status_bar_, LV_FLEX_FLOW_ROW); // 横向布局
    lv_obj_set_style_pad_all(status_bar_, 0, 0);
    lv_obj_set_style_border_width(status_bar_, 0, 0);
    lv_obj_set_style_pad_column(status_bar_, 0, 0);
    lv_obj_set_style_pad_left(status_bar_, 30, 0);
    lv_obj_set_style_pad_right(status_bar_, 30, 0);
    ...
    status_label_ = lv_label_create(status_bar_);
    lv_obj_set_flex_grow(status_label_, 1);
    lv_label_set_long_mode(status_label_, LV_LABEL_LONG_SCROLL_CIRCULAR); // 循环滚动文本
    lv_obj_set_style_text_align(status_label_, LV_TEXT_ALIGN_CENTER, 0);
    ...
    lv_obj_set_size(high_temp_popup_, LV_HOR_RES * 0.9, fonts_.text_font->line_height * 3);
    lv_obj_align(high_temp_popup_, LV_ALIGN_CENTER, 0, 0);  // 居中显示更醒目
    lv_obj_set_style_bg_color(high_temp_popup_, lv_palette_main(LV_PALETTE_RED), 0);
    lv_obj_set_style_radius(high_temp_popup_, 10, 0);
    lv_obj_set_style_border_width(high_temp_popup_, 3, 0);
    lv_obj_set_style_border_color(high_temp_popup_, lv_color_hex(0xFF0000), 0);
    high_temp_label_ = lv_label_create(high_temp_popup_);
    lv_label_set_text(high_temp_label_, "提示！\n请勿边充边用！");
    lv_obj_set_style_text_color(high_temp_label_, lv_color_white(), 0);
    lv_obj_set_style_text_align(high_temp_label_, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_center(high_temp_label_);
    lv_obj_add_flag(high_temp_popup_, LV_OBJ_FLAG_HIDDEN); // 默认隐藏

    /* -------------------- 根据保存的模式初始化显示 -------------------- */
    if (current_mode_) {
        // 宠物模式：显示 GIF，隐藏文字相关元素
        if (overlay_container != nullptr) {
            lv_obj_clear_flag(overlay_container, LV_OBJ_FLAG_HIDDEN);
        }
        if (gif_label_ != nullptr) {
            lv_gif_set_src(gif_label_, &idle2);
...
void LcdDisplay::SetPreviewImage(const lv_img_dsc_t* img_dsc) {
    ...
    if (img_dsc != nullptr) {
        ...
        if (gif_label_ != nullptr) {
            lv_obj_add_flag(gif_label_, LV_OBJ_FLAG_HIDDEN);
        }
    } else {
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    DisplayLockGuard lock(this);
    if (preview_image_ == nullptr || emotion_label_ == nullptr || chat_message_label_ == nullptr) {
        return;
    }
    ...
    /* -------------------- 预览图片 Preview Image -------------------- */
    lv_image_set_src(preview_image_, &LOGO);

    /* -------------------- 聊天消息 Chat Message -------------------- */
    lv_label_set_text(chat_message_label_, content);
    lv_obj_set_width(chat_message_label_, LV_HOR_RES - 10);   // 留内边距
    lv_obj_set_height(chat_message_label_, LV_SIZE_CONTENT);  // 高度自适应
    lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_WRAP); // 自动换行
    lv_obj_set_style_text_align(chat_message_label_, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_color(chat_message_label_, lv_color_white(), 0);
    lv_obj_set_style_text_font(chat_message_label_, &font_puhui_20_4, 0);
    ...
    lv_obj_clear_flag(chat_message_label_, LV_OBJ_FLAG_HIDDEN);      // 显示消息
    lv_obj_set_parent(chat_message_label_, content_);                // 保证在 content_ 下

    // 确保emotion_label_在最上层（如果它是可见的）
    if (!lv_obj_has_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN)) {
        lv_obj_move_foreground(emotion_label_);
    }
}
...
void LcdDisplay::SetEmotion(const char* emotion) {
    if (current_mode_) {
        ...
        if (preview_image_ != nullptr) {
            /* -------------------- 预览图片 Preview Image -------------------- */
            lv_image_set_src(preview_image_, &LOGO);
            lv_obj_clear_flag(preview_image_, LV_OBJ_FLAG_HIDDEN);  // 显示预览图片

            /* -------------------- 3. 隐藏 emotion 标签和 GIF -------------------- */
            lv_obj_add_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
            if (gif_label_ != nullptr) {
                lv_obj_add_flag(gif_label_, LV_OBJ_FLAG_HIDDEN);
            }
...
void LcdDisplay::SetIcon(const char* icon) {
...
#if !CONFIG_USE_WECHAT_MESSAGE_STYLE
    // 显示emotion_label_，隐藏preview_image_和gif_label_
    lv_obj_clear_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
    if (preview_image_ != nullptr) {
        lv_obj_add_flag(preview_image_, LV_OBJ_FLAG_HIDDEN);
    }
    ...
    // 确保emotion_label_在最上层
    lv_obj_move_foreground(emotion_label_);
#endif
}

void LcdDisplay::SetStatus(const char* status) {
    // 在配网模式或激活模式下，强制切换到文字模式显示
    if (status == Lang::Strings::WIFI_CONFIG_MODE || status == Lang::Strings::ACTIVATION) {
        if (current_mode_) {
            // 当前是宠物模式，需要切换到文字模式
            SetMode("normal");
        }
...
void LcdDisplay::SetMode(const std::string& mode_name) {
    DisplayLockGuard lock(this);
    if (mode_name == "normal" || mode_name == "NORMAL") {
        current_mode_ = false;
    } else if (mode_name == "pet" || mode_name == "PET") {
        current_mode_ = true;
        ...
        // Invalid mode name, return false
        ESP_LOGE(TAG, "Invalid mode name: %s", mode_name.c_str());
        return;
    }
    ...
    if (current_mode_) {
        if (container_ != nullptr) {
            lv_obj_set_style_bg_color(container_, lv_color_black(), 0); 
            lv_obj_set_style_bg_opa(container_, LV_OPA_COVER, 0); // 设置背景透明度
        }
        if (status_bar_ != nullptr){
            lv_obj_set_style_bg_color(status_bar_,lv_color_black(), 0);  // 黑色背景
            lv_color_t blue_green = lv_color_hex(0x0D98BA);  // 标准蓝绿色
            lv_obj_set_style_text_color(status_bar_,blue_green, 0);
            
        
            if (network_label_ != nullptr) {
                lv_obj_set_style_text_color(network_label_, blue_green, 0);
            }
            if (status_label_ != nullptr) {
                lv_obj_set_style_text_color(status_label_, blue_green, 0);
            }
            if (notification_label_ != nullptr) {
                lv_obj_set_style_text_color(notification_label_, blue_green, 0);
            }
            if (mute_label_ != nullptr) {
                lv_obj_set_style_text_color(mute_label_, blue_green, 0);
            }
            if (battery_label_ != nullptr) {
                lv_obj_set_style_text_color(battery_label_, blue_green, 0);
            }
        }
        if (emotion_label_ != nullptr) {
            lv_obj_add_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
        ...
        if (overlay_container != nullptr) {
            lv_obj_add_flag(overlay_container, LV_OBJ_FLAG_HIDDEN);
        }
        if (emotion_label_ != nullptr) {
            lv_obj_clear_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
            SetEmotion("neutral");
...
void LcdDisplay::ShowHighTempWarning() {
    DisplayLockGuard lock(this);
    if (high_temp_popup_) {
        if (lv_obj_has_flag(high_temp_popup_, LV_OBJ_FLAG_HIDDEN)) {
            ESP_LOGW(TAG, "Showing high temperature warning popup");
            lv_obj_clear_flag(high_temp_popup_, LV_OBJ_FLAG_HIDDEN);
        }
    } else {
        ESP_LOGE(TAG, "High temp popup not initialized!");
    }
}

void LcdDisplay::HideHighTempWarning() {
    DisplayLockGuard lock(this);
    if (high_temp_popup_) {
        if (!lv_obj_has_flag(high_temp_popup_, LV_OBJ_FLAG_HIDDEN)) {
            ESP_LOGI(TAG, "Hiding high temperature warning popup");
